...
class update{
    public function edit($conn){
        ...
        $this->output = '';  
            ...
            {  
                $query = "UPDATE users SET email='$this->email', password='$this->password', name='$this->name', 
                          vehicle='$this->vehicle' WHERE id='".$_POST["customer_id"]."'";  
                $this->output .= '<script>alert("Data has been updated");</script>'; 
            }
            if($conn->query($query))
            {
